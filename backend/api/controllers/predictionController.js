...
exports.experiencePrediction = async (req, res) => {
  ...
  try {
    ...
    if (!experience.trainedModel || !Buffer.isBuffer(experience.trainedModel)) {
      return res.status(400).json({ error: "Modelo entrenado no disponible o formato inválido." });
    }
    ...
    const predictionMatrixPath = path.join(userTempDir, "prediction_matrix.json");
    const predictionTensorPath = path.join(userTempDir,"prediction_tensor.json");

    fs.writeFileSync(predictionMatrixPath, JSON.stringify(response.data.matriz_prediccion));
    fs.writeFileSync(predictionTensorPath, JSON.stringify(response.data.tensor_prediccion));
